...
**A Novel Post-Training Quantization Framework for Compressing Large Language Models**

*Achieving extreme compression (4-bit / 3-bit / 2-bit) with minimal perplexity degradation*
...
- **Physical INT32 Bit-Packing** - Packs 8x (4-bit) or 10x (3-bit) weights into a single INT32 container, achieving real physical memory savings (not just simulated)
...
- **Export and Share** - Compress any HuggingFace model into a single portable `.pt` file that anyone can run
...
Evaluated on the official WikiText-2 Test Split (256 context chunks of 1024 tokens). Zero data leakage.
...
| GPT-2 Large | 774M | 1548 MB | 2-bit | 193 MB | 8.0x | 29.43 | 29.66 | +0.7% |
| TinyLlama 1.1B | 1.1B | 2200 MB | 3-bit | 412 MB | 5.3x | 14.48 | 14.51 | +0.2% |
...
| Precision | Model Size | Pass@1 | Delta vs FP16 |
|-----------|------------|--------|---------------|
| FP16 (Baseline) | 172.0 MB | 72.0% | 0.0% |
...
### Option 1: Compress and Export a HuggingFace Model
...
|-- tmgq_export_llama.py        # Export: Compress HuggingFace models to .pt file
...
```
 FP16/BF16 Model (Original HuggingFace weights)
        |
        v
 Group-wise MinMax Asymmetric Mapping
 Per-row, per-block (gs=128) boundary extraction
 w_int = round((w - b_min) / scale)
        |
        v
 INT32 Bit-Packing
 4-bit: 8 weights per INT32 (8x compression)
 3-bit: 10 weights per INT32 (10x compression)
        |
        v
...
```
 Load .pt Checkpoint
        ...
        v
 Detect INT32 Keys
 Automatically identifies packed QuantizedLinear layers
 Replaces nn.Linear with QuantizedLinear dynamically
        |
        v
 On-the-fly Unpack and Dequantize
 INT32 -> individual weights via bitwise operations
 w_float = (w_int * scale) + b_min
 Standard F.linear(x, w_dequantized, bias)
```
...
|----------|-------------|
| `pack_3bit(limits_int)` | Packs 10 x 3-bit integers into one INT32 |
| `unpack_3bit(packed, shape, pad)` | Extracts 3-bit integers from INT32 containers |
| `pack_4bit(limits_int)` | Packs 8 x 4-bit integers into one INT32 |
| `unpack_4bit(packed, shape, pad)` | Extracts 4-bit integers from INT32 containers |
| `extract_packed_schema(w, n_bits, gs)` | Quantizes float weights to INT limits + scales + zeros |
...
1. **Hessian Diagonal Extraction**: Calibrate the model using forward passes on sampled texts to approximate the diagonal of the Hessian matrix. This yields a per-weight sensitivity profile.

2. **Error Diffusion Waves**: The residual quantization error (E = W_orig - W_quant) is re-dispersed across the remaining weights in the same layer tensor, scaled strictly by the measured Hessian trace.
